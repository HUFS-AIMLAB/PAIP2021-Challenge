...
def parse_args():
    ...
    parser.add_argument(
        "--train_mode", type=str, help="clf: classification, seg: segmentation"
    )
    parser.add_argument(
        "--train_type", type=str, help="encoder, [for probmap]: col, pan, pros"
    )
...
def main():
    ...
    if args.train_mode == "clf":
        ...
        model = timm.create_model(
            "tf_efficientnet_b0_ns", pretrained=True, num_classes=4
        )
        criterion = torch.nn.CrossEntropyLoss()
    elif args.train_mode == "seg":
        encoder_path = os.path.join(
            args.model_dir, "clf/all", f"level_{args.level}/checkpoint.pt"
        )
        model = smp.Unet(
        ...
        criterion = monai.losses.DiceLoss(
            softmax=True, to_onehot_y=True, include_background=True
        )
    print(model)
