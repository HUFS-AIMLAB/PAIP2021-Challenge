    ...
    def make_patch(self, slide, mask, name, level):
        ...
        for idx in idx_list:
            ...
            for i in range(range_max):
                ...
                mask_ = mask[coord_y[num][i] - (args.psize//2) : coord_y[num][i] + (args.psize//2), coord_x[num][i] - (args.psize//2) : coord_x[num][i] + (args.psize//2), np.newaxis]
                mask_ = mask_.astype(np.uint8)
                
                
                num2 = format(cnt, '07')
                cnt += 1
                ipath = Path(os.path.join(self.patch_save, name, str(level),f"{label}/img", f"{name}_{num2}.png"))
                mpath = Path(os.path.join(self.patch_save, name, str(level),f"{label}/mask", f"{name}_{num2}.png"))
