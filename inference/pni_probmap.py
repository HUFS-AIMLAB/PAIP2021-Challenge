...
class Inferer:
    ...
    def read_wsi_clf(self, slide):
        ...
        steps_x = int(
            math.ceil((max_x - overlap[0]) / float(self.psize[0] - overlap[0]))
        )
        steps_y = int(
            math.ceil((max_y - overlap[1]) / float(self.psize[1] - overlap[1]))
        )
        start_coords = list()
        ...
        for y in tqdm(range(0, steps_y)):
            for x in range(0, steps_x):
                ...
                temp = np.array(
                    slide.read_region(
                        (x_start * (4 ** (self.level)), y_start * (4 ** (self.level))),
                        self.level,
                        self.psize,
                    )
                )
                ...
                if num == self.batch_size or (y == steps_y - 1 and x == steps_x - 1):
                    ...
                    for i in range(length):
                        ...
                        overlay[
                            start_coords[i][0] : start_coords[i][0]
                            + self.psize[0] // denom,
                            start_coords[i][1] : start_coords[i][1]
                            + self.psize[1] // denom,
                        ] += pred_fill
                        reference[
                            start_coords[i][0] : start_coords[i][0]
