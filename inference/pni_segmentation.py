...
class SegInferer():
    ...
    def read_wsi_seg(self, slide, heatmap):
        ...
        steps_x = int(math.ceil((max_x - self.overlap[0]) / float(self.psize[0] - self.overlap[0])))
        steps_y = int(math.ceil((max_y - self.overlap[1]) / float(self.psize[1] - self.overlap[1])))
        start_coords = list()
        ...
        for y in tqdm(range(0,steps_y)):
            for x in range(0, steps_x):
                ...
                heatmap_patch = heatmap[y_start:y_start+self.psize[0], x_start:x_start+self.psize[1]]
                ...
                if np.amax(heatmap_patch) == 1:
                    ...
                    if num == self.batch_size or (y == steps_y - 1 and x == steps_x - 1):
                        ...
                        for i in range(length):
                            overlay[start_coords[i][0]:start_coords[i][0]+self.psize[0], start_coords[i][1]:start_coords[i][1]+self.psize[1]] += pred[i]
                            reference[start_coords[i][0]:start_coords[i][0]+self.psize[0], start_coords[i][1]:start_coords[i][1]+self.psize[1]] += 1
                            if i == length-1:
                                num = 0
                                start_coords = []
