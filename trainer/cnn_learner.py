...
class EncoderTrainer:
    ...
    def path_list(self):
        ...
        for patient in sorted(os.listdir(os.path.join(root_dir))):
            ...
            for label in sorted(
                os.listdir(
                    os.path.join(root_dir, patient, "random", f"level_{level_dim}")
                )
            ):
                if "class" in label:
                    for image in sorted(
    ...
    def validation(self, valid_loader, length):
        ...
        with torch.no_grad():
            for item in tqdm(valid_iterator):
                iter_count += 1
                image, label = (
                    item["image"].to(self.device),
                    item["label"].type(torch.long).to(self.device),
                )
    ...
    def training(self):
        ...
        for epoch in range(1, self.args.train_epochs + 1):
            ...
            print(
                f"Epoch: {epoch} | Valid Loss: {valid_loss:.3f}, Valid Acc: {valid_acc:.3f}"
            )
