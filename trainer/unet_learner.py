...
def strong_aug(p=0.5):
    return A.Compose(
        [
            ...
            A.ShiftScaleRotate(
                shift_limit=0.1, scale_limit=0.2, rotate_limit=180, p=0.5
            ),
            A.OneOf([A.ElasticTransform(p=0.5),], p=0.5),
            A.OneOf(
                [
            ...
            A.OneOf([A.GaussNoise(var_limit=[1000, 1050]),], p=0.7),
...
class UNetTrainer:
    ...
    def path_list(self):
        ...
        for patient in sorted(os.listdir(os.path.join(root_dir))):
            if patient.split("_")[0] not in ["Col", "Pan", "Pros"]:
                continue
            if (
                self.args.train_type != "all"
                and patient.split("_")[0] != self.args.train_type.title()
            ):
                continue
            for image in sorted(
                os.listdir(
                    os.path.join(
                        root_dir, patient, "sw", f"level_{level_dim}", "pni", "img_sw"
                    )
                )
            ):
    ...
    def validation(self, valid_loader, dice_metric, activate, discrete, inferer):
        ...
        with torch.no_grad():
            for item in tqdm(valid_iterator):
                iter_count += 1
                image, label = (
                    item[0].to(self.device),
                    item[1].type(torch.long).to(self.device),
                )
                pred = inferer(inputs=image, network=self.model)
                loss = self.criterion(pred, label)
                valid_iterator.set_description(
                    "Training (%d / %d Steps) (loss=%2.5f)"
                    % (iter_count, max_iterations, loss.item())
                )
                pred = activate(pred)
                pred = discrete(pred)
    ...
    def training(self):
        ...
        train_list, valid_list = train_test_split(
            data_list, test_size=0.1, shuffle=True
        )
        print(f"[INFO] train_list: {len(train_list)}, valid_list: {len(valid_list)}")

        albu_aug = strong_aug(p=0.8)
        train_aug = A.Compose([albu_aug, A.Normalize(), ToTensorV2(),])
        valid_aug = A.Compose([A.Normalize(), ToTensorV2(),])
        ...
        for epoch in range(1, self.args.train_epochs + 1):
            ...
            valid_loss, valid_dice = self.validation(
                valid_loader, dice_metric, activate, discrete, inferer
            )

            print(f"Epoch: {epoch} | Train Loss: {train_loss:.3f}")
            print(
                f"Epoch: {epoch} | Valid Loss: {valid_loss:.3f}, Valid Dice Score: {valid_dice:.3f}"
            )
